...
struct Sequence2 {
    Sequence2() = default;
    explicit Sequence2(size_t width, size_t height) : width(width), height(height), data(width*height), radius(width/2) {
    }
...
struct ContFilter2 {
    ContFilter2() = default;
    ContFilter2(size_t radius,  std::function<Float(Float x, Float y)> func) : radius(radius), func(func) {}
...
static Float reconstruct(Sequence2 seq, ContFilter2 filter, Float x, Float y) {
    ...
    for (int i = xstart; i <= xend; ++i) {
        for (int j = ystart; j <= yend; ++j) {
            if (i < 0 || i >= seq.getWidth()) continue;
            if (j < 0 || j >= seq.getHeight()) continue;
...
static Float catmullRom1(Float x) {
    if (-1.0 <= x && x <= 1.0) {
        auto out = (-3*pow(1.0-fabs(x), 3.0) + 4*pow(1.0-fabs(x),2.0) + (1.0-fabs(x)))/2.0;
        return out;
    } else {
...
static Float mitchell1(Float x) {
    if (-1.0 <= x && x <= 1.0) {
        auto out = (-21*pow(1.0-fabs(x), 3.0) + 27*pow(1.0-fabs(x),2.0) + 9*(1.0-fabs(x))+1)/18.0;
        return out;
    } else {
...
static ContFilter2 normalFilter2(Float std, Float r) {
    ...
    return ContFilter2(r, [=](Float x, Float y) {
        return c * exp(-(x*x+y*y)/2);
    });
}
